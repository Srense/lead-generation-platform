                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12 items-start">
                    <div className="md:col-span-5 grid grid-cols-1 gap-8">
                        ...
                        <div className="floral-glass rounded-3xl p-8 md:p-10 ambient-shadow hover:-translate-y-2 transition-transform duration-300 border border-white/5">
                            ...
                            <address className="text-on-surface font-sans text-base not-italic leading-loose font-medium">
                                Chandigarh<br />
                                Mohali
                            </address>
                        </div>
                            ...
                            <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="flex flex-col gap-2">
                                        <label className="font-sans font-semibold text-xs tracking-wider uppercase text-on-surface-variant ml-1">First Name</label>
                                        <input required name="firstName" value={formData.firstName} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl py-3.5 px-4 text-on-surface text-base focus:border-primary focus:bg-black/60 focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-white/20" placeholder="Jane" type="text" />
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        <label className="font-sans font-semibold text-xs tracking-wider uppercase text-on-surface-variant ml-1">Last Name</label>
                                        <input required name="lastName" value={formData.lastName} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl py-3.5 px-4 text-on-surface text-base focus:border-primary focus:bg-black/60 focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-white/20" placeholder="Doe" type="text" />
                                    </div>
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label className="font-sans font-semibold text-xs tracking-wider uppercase text-on-surface-variant ml-1">Email Address</label>
                                    <input required name="email" value={formData.email} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl py-3.5 px-4 text-on-surface text-base focus:border-primary focus:bg-black/60 focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-white/20" placeholder="jane.doe@example.com" type="email" />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label className="font-sans font-semibold text-xs tracking-wider uppercase text-on-surface-variant ml-1">Inquiry Type</label>
                                    <div className="relative">
                                        <select name="inquiryType" value={formData.inquiryType} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl py-3.5 pl-4 pr-10 text-on-surface text-base focus:border-primary focus:bg-black/60 focus:ring-1 focus:ring-primary outline-none transition-all appearance-none cursor-pointer">
                                            <option value="support" className="bg-surface text-on-surface">General Support</option>
                                            <option value="sales" className="bg-surface text-on-surface">Sales &amp; Partnerships</option>
                                            <option value="press" className="bg-surface text-on-surface">Press &amp; Media</option>
                                        </select>
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="font-sans font-semibold text-xs tracking-wider uppercase text-on-surface-variant ml-1">Message</label>
                                    <textarea required name="message" value={formData.message} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl py-3.5 px-4 text-on-surface text-base focus:border-primary focus:bg-black/60 focus:ring-1 focus:ring-primary outline-none transition-all resize-none placeholder:text-white/20" placeholder="How can we help you today?" rows="5"></textarea>
                                </div>
                                <button disabled={isSubmitting} type="submit" className="mt-6 bg-primary text-black font-sans font-bold uppercase tracking-wider py-4 px-8 rounded-xl hover:bg-primary-container hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all duration-300 disabled:opacity-50 disabled:hover:shadow-none">
